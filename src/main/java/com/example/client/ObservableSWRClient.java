 ...
 * are notified. This class contains two blocking methods that can be used when
 * a user wishes to send a message and then wait for a reply from the server.
 *
...
public class ObservableSWRClient extends ObservableClient {
	...
	/**
	 * Sets the wait time. At the end of each wait time period, the instance will
	...
	/**
	 * Connects to the server and waits. This method will block until the server
	 * confirm connection. At the end of each wait time period, the instance will
	 * notify its observers with the WAITING_FOR_REPLY message.
	 *
	...
	/**
	 * Sends a message to the server and waits for a reply. This method will block
	 * until the server sends the expected reply. At the end of each wait time
	 * period, the instance will notify its observers with the WAITING_FOR_REPLY
	 * message.
	 *
	 * @param message        The message sends to the server.
	 * @param expectedObject The client will wait until it receives an object equals
	 *                       to this one.
	...
	/**
	 * Sends a message to the server and waits for a reply. This method will block
	 * until the server sends one of the expected list of replies. At the end of
	 * each wait time period, the instance will notify its observers with the
	 * WAITING_FOR_REPLY message.
	 *
	 * @param message              The message sends to the server.
	 * @param expectedListOfObject The client will wait until it receives an object
	 *                             equals to one of the objects in this list.
	 * @return the object received.
	 * @exception IOException if an I/O error occurs.
	 */
