...
/**
 * This class acts as a subclass of <code>AbstractServer</code> and is also an
 * <code>Observable</code> class. This means that when a message is received,
...
// ACCESSING METHODS ------------------------------------------------
	...
	/**
	 * Returns an array of containing the existing client connections. This can be
	 * used by concrete subclasses to implement messages that do something with each
	 * connection (e.g. kill it, send a message to it etc.)
	 *
	...
	/**
	 * Sets the port number for the next connection. Only has effect if the server
	 ...
	 * second. The server must be stopped and restarted for the timeout change be in
	 * effect.
	 *
	...
	/**
	 * Hook method called each time an exception is raised in a client thread. This
	 * implementation simply closes the client connection, ignoring any exception.
	...
	/**
	 * This method is called when the server stops accepting connections because an
	 * exception has been raised. This implementation simply calls
	...
	/**
	 * This method is called when the server is closed. This method may be overriden
	 * by subclasses.
	 */
	...
	/**
	 * This method is called when the server starts listening for connections. The
	 ...
	 * notfied by receiveing the transmitted message. Note that, in this
	 * implementation, the information concerning the client that sent the message
	 * is lost. It can be overriden, but is still expected to call
	 * notifyObservers().
	 *
